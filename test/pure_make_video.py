...
def images_to_video():
    base_path = "/share/czh/splinegs_0902"
    scenes = ["block"]
    output_video = os.path.join(base_path, "val_video.mp4")
    fps = 10  
    image_paths = []
    for scene in scenes:
    ...
    for img_path in image_paths:
        ...
        if img is None:
            ...
            continue
        # 先裁剪左边1/3再计算最大尺寸
        cropped_img = crop_left_third(img)
        height, width = cropped_img.shape[:2]
        max_width = max(max_width, width)
        max_height = max(max_height, height)
        valid_images.append(img_path)

    print(f"检测到裁剪后最大图像尺寸: {max_width}x{max_height}，将以此为标准尺寸")
    ...
    for i, img_path in enumerate(valid_images):
        ...
        cropped_img = crop_left_third(img)
        # 为裁剪后的图像添加边框以统一尺寸
        img_with_border = add_white_border(cropped_img, max_width, max_height)
